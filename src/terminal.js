...
function toggleMusic() {
  if (musicPlaying) {
    ...
    try {
      ...
      musicPlaying = true;
      console.log(
        `${colors.brightYellow}Musique de fond activée.${colors.reset}`
      );
    } catch (error) {
...
function displayBanner() {
  ...
  console.log(
    `${colors.brightCyan}║       ${colors.brightYellow}CODE BREAKER - TERMINAL${colors.brightCyan}       ║${colors.reset}`
  );
  console.log(
    `${colors.brightCyan}╚═════════════════════════════════════╝${colors.reset}`
  );
...
function displayInstructions() {
  ...
  console.log(
    `- ${colors.brightGreen}Vert${colors.reset} = bon chiffre, bonne position`
  );
  console.log(
    `- ${colors.brightYellow}Orange${colors.reset} = bon chiffre, mauvaise position`
  );
...
function startGame() {
  ...
  rl.question(
    `${colors.brightCyan}Entrez votre nom: ${colors.reset}`,
    (name) => {
      ...
      console.log(
        `${colors.brightWhite}Devinez le code secret de 3 chiffres...${colors.reset}\n`
      );
...
function askForGuess() {
  rl.question(
    `${colors.brightCyan}Votre proposition (#${attempts.length + 1}): ${
      colors.reset
    }`,
    (input) => {
      ...
      if (!/^\d{3}$/.test(guess)) {
        console.log(
          `${colors.brightRed}Entrée invalide! Veuillez entrer exactement 3 chiffres.${colors.reset}\n`
        );
        askForGuess();
        return;
      }
      ...
      attempts.forEach((attempt, index) => {
        console.log(
          `#${index + 1}: ${displayFeedback(attempt.guess, attempt.feedback)}`
        );
      });
      console.log();
      ...
      if (feedback.every((f) => f === "green")) {
        ...
        console.log(
          `${colors.brightGreen}║           FÉLICITATIONS!            ║${colors.reset}`
        );
        console.log(
          `${colors.brightGreen}╚═════════════════════════════════════╝${colors.reset}`
        );
        console.log(
          `${colors.brightGreen}Vous avez trouvé le code en ${
            attempts.length
          } essai${attempts.length > 1 ? "s" : ""}!${colors.reset}`
        );

        const score = 100 - attempts.length * 5;
        console.log(
          `${colors.brightYellow}Votre score: ${Math.max(score, 10)} points${
            colors.reset
          }\n`
        );
        ...
        console.log(
          `${colors.brightRed}║           PARTIE TERMINÉE           ║${colors.reset}`
        );
        console.log(
          `${colors.brightRed}╚═════════════════════════════════════╝${colors.reset}`
        );
        console.log(
          `${colors.brightRed}Vous avez atteint le nombre maximum d'essais.${colors.reset}`
        );
        console.log(
          `${colors.brightWhite}Le code secret était: ${
            colors.brightMagenta
          }${secretCode.join("")}${colors.reset}\n`
        );
...
function askToPlayAgain() {
  rl.question(
    `${colors.brightCyan}Voulez-vous rejouer? (o/n): ${colors.reset}`,
    (answer) => {
      if (answer.toLowerCase() === "o" || answer.toLowerCase() === "oui") {
        ...
        displayBanner();
        console.log(
          `${colors.brightMagenta}Bienvenue à nouveau, ${playerName}!${colors.reset}`
        );
        console.log(
          `${colors.brightWhite}Devinez le code secret de 3 chiffres...${colors.reset}\n`
        );
        ...
        console.log(
          `${colors.brightWhite}Consultez vos scores dans l'interface web.${colors.reset}\n`
        );
        cleanup();
      }
    }
